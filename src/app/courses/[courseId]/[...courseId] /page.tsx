...
const fetchData = async () => {
  const query =
    "https://y8rjsgga.api.sanity.io/v2022-03-07/data/query/production?query=*%5B_type+%3D%3D+%27course%27%5D+%7B%0A++name%0A%7D";
...
export default async function Page({
  ...
  const header = result.filter((category: any) => {
    console.log(
      "category.name",
      category.name.toLocaleLowerCase().replaceAll(" ", "-")
    );
    return (
      category.name.toLocaleLowerCase().replaceAll(" ", "-") ===
      params.courseId[0]
